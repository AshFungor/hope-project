...
#             account = StaticTablesHandler.prepare_bank_account(**models.BankAccount.make_spec("prefecture", prefecture))
#             if account is None:
#                 return f"error generating bank account for index {index}"

#             parsed.append(models.Prefecture(prefecture_name, account, None, None, None))
...
#             city_name, city_location, city_prefecture = city.get("Name", None), city.get("Location", None), city.get("Prefecture", None)
...
#             prefecture = env.db.impl().session.query(models.Prefecture).filter(models.Prefecture.name == city_prefecture).first()
#             if prefecture is None:
#                 return f"failed to find prefecture with name {city_prefecture}"
#             prefecture_id = prefecture.id

#             bank_account = StaticTablesHandler.prepare_bank_account(**models.BankAccount.make_spec("city", city))
#             if bank_account is None:
#                 return f"error generating bank account for index {index}"

#             parsed.append(models.City(city_name, None, prefecture_id, bank_account, city_location))
...
#                 user.get("Name", None),
#                 user.get("Surname", None),
#                 user.get("Patronymic", None),
#                 user.get("Login", None),
#                 user.get("City", None).strip(),
#                 user.get("Password", None),
#                 user.get("Sex", None),
#                 user.get("Birthday", None),
#                 user.get("Bonus", None),
#                 user.get("IsAdmin", None),
#             )
...
#             user_city_obj = env.db.impl().session.query(models.City.id).filter(models.City.name == user_city).first()
#             if user_city_obj is None:
#                 return f"failed to find city with name {user_city}"
#             user_city_id = user_city_obj.id

#             bank_account = StaticTablesHandler.prepare_bank_account(**models.BankAccount.make_spec("user", user))
#             if bank_account is None:
#                 return f"error generating bank account for index {index}"
...
#     @staticmethod
#     def prepare_products_to_bank_account(bank_account_id: int, product_id: int, count: int) -> None:
#         product_to_bank_account = models.Product2BankAccount(bank_account_id, product_id, count)
#         env.db.impl().session.add(product_to_bank_account)

#     @staticmethod
#     def __prepare_products(products: pd.DataFrame) -> int | str:
#         existing = [np.asarray(env.db.impl().session.query(models.Product.name).all(), dtype="str"), np.ones(0), np.ones(0)]
#         parsed = []

#         for index, product in products.iterrows():
#             product_name, product_category, product_level = product.get("Name", None), product.get("Category", None), product.get("Level", None)
...
#             parsed.append(models.Product(product_category, product_name, int(product_level)))

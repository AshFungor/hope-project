...
def users() -> list[int]:
    accounts = env.db.impl().session.query(models.BankAccount).join(models.User, models.BankAccount.id == models.User.bank_account_id)
...
def high_rule() -> list[int]:
    # exclude admins
    admins = env.db.impl().session.query(models.User).filter(models.User.is_admin == True).all()
    # exclude companies by admins
    companies = (
        env.db.impl()
        .session.query(models.Company)
        .join(models.User2Company, models.User2Company.company_id == models.Company.id)
        .filter(models.User2Company.user_id.in_([admin.id for admin in admins]))
        .all()
    )
    # exclude cities where admins live
    cities = env.db.impl().session.query(models.City).filter(models.City.id.in_([admin.city_id for admin in admins])).all()

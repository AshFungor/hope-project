...
def select_consumption_total(
    ...
    auto_margin: datetime.timedelta = datetime.timedelta(hours=4) - datetime.timedelta.resolution,
) -> list[int]:
    if (datetime.datetime.combine(datetime.date.today(), auto_lower) + auto_margin).time() < auto_lower:
        raise ValueError(f"upper should not exceed 1 day")
    return [
        consumption.defaults[product.category] * consumed.count
        for consumed, product in env.db.impl()
        .session.query(models.Consumption, models.Product)
        .join(models.Product, models.Product.id == models.Consumption.product_id)
        .filter(
            orm.and_(
                models.Consumption.bank_account_id == id,
                models.Consumption.consumed_at >= (datetime.datetime.now() - offset),
                orm.cast(models.Consumption.consumed_at, orm.Time) >= auto_lower,
                orm.cast(models.Consumption.consumed_at, orm.Time)
                <= (datetime.datetime.combine(datetime.date.today(), auto_lower) + auto_margin).time(),
            )
        )
        .all()
    ]
...
def select_spending_total(id: int, offset: datetime.timedelta) -> list[int]:
    return [
        transaction.amount
        for transaction in env.db.impl()
...
def select_gain_total(id: int, offset: datetime.timedelta) -> list[int]:
    return [
        transaction.amount
        for transaction in env.db.impl()
        .session.query(models.Transaction)
...
def restore_account_delta(id: int, offset: datetime.timedelta, *query_args: any) -> int:
...
def calculate_total_market_share(include_bankrupts: bool = False, offset: datetime.timedelta | None = None) -> list[typing.Tuple[int, int]]:
    """
    ...
    return np.asarray(
        [
            (account.bank_account_id, account.count - restore_account_delta(account.bank_account_id, offset))
            for account in env.db.impl()
            .session.query(
                models.Product2BankAccount,
            )
            .filter(
                orm.and_(
                    (orm.true if include_bankrupts else models.Product2BankAccount.count >= 0),
                    models.Product2BankAccount.product_id == 1,
                    models.Product2BankAccount.bank_account_id.not_in(exclude.high_rule()),
                )
            )
            .all()
        ]
...
def calculate_external_loss(offset: datetime.timedelta | None = None) -> typing.Tuple[list[typing.Tuple[int, int]], list[typing.Tuple[int, int]]]:
    """
    ...
    spent = [
        ...
        .filter(
            orm.and_(
                ...
                models.Transaction.customer_bank_account_id.in_(exclude.high_rule()),
                models.Transaction.seller_bank_account_id.not_in(exclude.high_rule()),
                models.Transaction.status == "approved",
                (orm.true if offset is not None else models.Transaction.updated_at <= datetime.datetime.now() - offset),
            )
        )
        .all()
    ]
    ...
    consumed = [
        (account.id, np.sum(select_consumption_total(account.id, offset))) for account in env.db.impl().session.query(models.BankAccount).all()
    ]

    return np.asarray(spent), np.asarray(consumed)


def calculate_external_gain(offset: datetime.timedelta | None = None) -> int:
    """
    ...
    return np.asarray(
        [
            ...
            .filter(
                orm.and_(
                    models.Transaction.product_id == 1,
                    models.Transaction.seller_bank_account_id.in_(exclude.high_rule()),
                    models.Transaction.customer_bank_account_id.not_in(exclude.high_rule()),
                    (True == True if offset is not None else models.Transaction.updated_at <= datetime.datetime.now() - offset),
                )
            )
            .all()
        ]
...
def get_company_income(
    account: int, with_exact_timestamps: bool = False, exclude_government: bool = True, offset: datetime.timedelta | None = None
    ...
    transactions = (
        ...
        .filter(orm.or_(models.Transaction.customer_bank_account_id == account, models.Transaction.seller_bank_account_id == account))
        .filter(
            orm.and_(
                models.Transaction.status == "approved",
                (
                    ...
                    else models.Transaction.customer_bank_account_id.not_in([int(id) for id in common.get_government()])
                ),
                (True == True if offset is None else models.Transaction.updated_at >= datetime.datetime.now() - offset),
            )
        )
        .order_by(orm.asc(models.Transaction.updated_at))
        .all()
    )
    ...
    relative = np.min([transaction.updated_at for transaction, _ in transactions])
    approximate_timestamp = lambda timestamp: (timestamp - relative).total_seconds() / (60 * 60)
...
def plot_income(title: str, id: int | None = None) -> str:
    ids = [
        ...
        for company in env.db.impl().session.query(models.Company).filter(models.Company.bank_account_id.not_in(exclude.high_rule())).all()
    ]
    if id is not None:
        ids = [id]
...
def view_statistics():
    ...
    plotted_share, plotted_gain, plotted_spent = plot_pie(share, share_title), plot_pie(gained, gained_title), plot_pie(spent, spent_title)

    companies = env.db.impl().session.query(models.Company).filter(models.Company.bank_account_id.not_in(exclude.high_rule())).all()

    return flask.render_template(
        "main/view_income.html", spec=spec, plotted_share=plotted_share, plotted_loss=plotted_spent, plotted_gain=plotted_gain, companies=companies
    )
...
def view_company_statistics():
    ...
    company = int(company)
    obj = env.db.impl().session.query(models.Company).filter(models.Company.bank_account_id == company).first()

    income = plot_income(f"Прибыль компании {obj.name}", company)
    _, incomes, gains, products = get_company_income(company)
    _, shifted_incomes, shifted_gains, shifted_products = get_company_income(company, offset=datetime.timedelta(days=1, hours=12))
    overall, shifted = incomes[-1], shifted_incomes[-1]

    gains, shifted_gains = np.asarray(gains), np.asarray(shifted_gains)
    positives_overall, positives_shifted = np.sum(gains[gains > 0]), np.sum(shifted_gains[shifted_gains > 0])
    negatives_overall, negatives_shifted = np.sum(gains[gains < 0]), np.sum(shifted_gains[shifted_gains < 0])

    def aggregate(products):
        distributed = {}
        for count, name in products:
            if name not in distributed:
                distributed[name] = {"away": 0, "in": 0}
            if count < 0:
    ...
    distributed = {"total": aggregate(products), "shifted": aggregate(shifted_products)}
    ...
    return flask.render_template("main/view_company_income.html", plot=income, company_spec=company_spec, distributed=distributed)

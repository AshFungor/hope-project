...
def user_sugs(page=1):
    ...
    if request.method == 'POST':
        ...
        result_transaction = StaticTablesHandler.complete_transaction(request.form.get('transaction_id'), status)
        # result_transaction: (completed: bool, message: str)

        if result_transaction[0]:
            flask.flash(result_transaction[1])

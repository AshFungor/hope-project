...
def get_city(city_id: int) -> models.City | None:
    return env.db.impl().session.execute(sqlalchemy.select(models.City).filter_by(id=city_id)).scalars().first()


def get_city_residents(city_id: int) -> typing.Tuple[models.User] | None:
    return (
        env.db.impl()
        .session.execute(sqlalchemy.select(models.User.name, models.User.last_name, models.User.bank_account_id).filter_by(city_id=city_id))
        .all()
    )
...
def get_products_and_count(bank_account: int) -> typing.Tuple[models.Product] | None:
    return (
        ...
        .session.execute(
            ...
            .filter(models.Product2BankAccount.bank_account_id == bank_account)
        )
        .all()
    )
...
def get_city_companies(city_id: int) -> typing.Tuple[models.Company] | None:
    return (
        env.db.impl()
        .session.execute(
            sqlalchemy.select(models.Company.name)
...
def get_city_prefecture(prefecture_id: int) -> models.Prefecture | None:
    return env.db.impl().session.execute(sqlalchemy.select(models.Prefecture.name).filter_by(id=prefecture_id)).scalars().first()
...
def city_cabinet():
    ...
    if city is None:
        logger.warning(f"an error occurred while rendering the city page; " f"reason: city was not found to represent the page")
        return flask.abort(400, descripstion="An error occurred while rendering the city page")
    prefecture = get_city_prefecture(prefecture_id=city.prefecture_id)
    if prefecture is None:
        logger.warning(f"an error occurred while rendering the city page; " f"reason: prefecture was not found to represent the page")

...
# import app.routes.accounts.consumption as consumption
...
#         products = (
#             env.db.impl()
#             .session.query(models.Product, models.Product2BankAccount)
#             .filter(
#                 orm.and_(
...
#                 )
#             )
#             .join(models.Product, models.Product2BankAccount.product_id == models.Product.id)
#             .filter(models.Product2BankAccount.count != 0)
#         )
...
#     products = env.db.impl().session.execute(orm.select(models.Product).order_by(orm.desc(models.Product.level))).scalars().all()
#     categories = set([product.category for product in products])

#     return flask.render_template("main/view_all_products.html", products=products, categories=categories)

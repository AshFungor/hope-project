...
# # local
# import app.routes.accounts.personal as account



# @blueprints.accounts.route("/company_worker_employment", methods=["GET", "POST"])
# @login_required
...
#     company = env.db.impl().session.execute(sqlalchemy.select(models.Company).filter(models.Company.id == company_id)).scalars().first()
#     if company is None:
...
#         # проверяем
#         query_user = sqlalchemy.select(models.User).filter(models.User.bank_account_id == worker_bank_account_id)
...
#             models.User2Company(
#                 user_id=user.id, company_id=int(company_id), role=role, ratio=0, employed_at=datetime.datetime.now(tz=CurrentTimezone)
#             )
#         )
...
# @blueprints.accounts.route("/company_worker_fire")
# @login_required
...
#     if company_id:
#         worker_query = sqlalchemy.select(models.User2Company).filter(
#             sqlalchemy.and_(
#                 models.User2Company.user_id == user_id, models.User2Company.company_id == company_id, models.User2Company.role == user_role
#             )
#         )
...
# @blueprints.accounts.route("/company_workers")
# @login_required
# def company_workers():
...
#             sqlalchemy.or_(models.User2Company.role == "CEO", models.User2Company.role == "founder"),
#         )
#     )
#     company_id = env.db.impl().session.execute(query_companies).scalars().first()
#     # ещё надо сделать проверку, имеет ли право текущий пользователь работать с сотрудниками
...
#         query = (
#             sqlalchemy.select(
#                 models.User.name, models.User.last_name, models.User.patronymic, models.User.bank_account_id, models.User.id, models.User2Company.role
#             )
#             .join(models.User2Company, models.User.id == models.User2Company.user_id)
#             .filter(sqlalchemy.and_(models.User2Company.company_id == company_id, models.User2Company.fired_at == None))
#         )

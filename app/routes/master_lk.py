...
def action_with_product():
    ...
    product = env.db.impl().session.execute(
        sqlalchemy.select(
            models.Product
        )
        .filter_by(name=data['название товара'])
    ).scalars().first()
    if product is None:
    ...
    products2bank_account = env.db.impl().session.execute(
        ...
        .filter(sqlalchemy.and_(
            models.Product2BankAccount.product_id == product.id,
            models.Product2BankAccount.bank_account_id == data['счет']
        ))
    ).scalars().first()
    if products2bank_account is None:
        products2bank_account = models.Product2BankAccount(
            bank_account_id=int(data['счет']),
            product_id=product.id,
            count=0
        )
        env.db.impl().session.add(products2bank_account)
        env.db.impl().session.commit()
    customer_wallet = env.db.impl().session.execute(
        sqlalchemy.select(
            models.Product2BankAccount
        )
        .filter_by(bank_account_id=data['счет'], product_id=1)
    ).scalars().first()
    if form_id == 3:
...
def add_money():
    try:
        ...
        return flask.Response(
            'Ошибка при добавлении надиков',
            400
        )
    user_wallet = env.db.impl().session.execute(
        sqlalchemy.select(
            models.Product2BankAccount
        )
        .filter_by(bank_account_id=data['счет'], product_id=1)

...
def make_company(name: str, about: str, prefecture_name: str, user_id: int) -> models.Company | None:
    ...
    bank_account = static.StaticTablesHandler.prepare_bank_account(
        **models.BankAccount.make_spec("company", {"name": name, "about": about, "prefecture_name": prefecture_name})
    )

    try:
        return models.Company(bank_account, prefecture.id, name, about)
...
def add_company(name: str, about: str, founders: list[int], prefecture_name: str) -> typing.Tuple[bool, str]:
    ...
    ids = env.db.impl().session.query(models.User).filter(models.User.bank_account_id.in_(founders)).all()
    ...
    for founder, ratio in zip(ids, ratios):
        env.db.impl().session.add(models.User2Company(founder.id, company.id, "founder", ratio, datetime.datetime.now(tz=CurrentTimezone)))
...
def create_company():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        founders = list(map(int, form.founders.data.strip().split()))  # founders - это список расчётных счетов учредителей фирмы

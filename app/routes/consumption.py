...
norms = {"FOOD": 1, "CLOTHES": 1, "TECHNIC": 1}

defaults = {"FOOD": 120, "CLOTHES": 160, "TECHNIC": 200}

time_accounted = {"FOOD": datetime.timedelta(days=1), "CLOTHES": datetime.timedelta(days=2), "TECHNIC": datetime.timedelta(days=4)}
...
def collect_consumers(data: typing.Tuple[models.User, dict[str, str]], full_mapping: dict[str, str]) -> typing.Tuple[bool, str]:
    fillers = {}
    for mapped in full_mapping:
        ...
        match = env.db.impl().session.query(models.Product).filter(orm.and_(models.Product.category == mapped, models.Product.level == 1)).first()
        if match is None:
    ...
    for user, mappings, _ in data:
        for mapped in mappings:
            ...
            try:
                status, _ = models.Consumption.did_consume_enough(user.bank_account_id, mapped, norms[mapped], time_accounted[mapped])
                if status:
                    continue
                env.db.impl().session.add(
                    models.Consumption(
                        user.bank_account_id,
                        fillers[mapped].id,
                        norms[mapped],
                        datetime.datetime.now(tz=CurrentTimezone) - datetime.timedelta(days=1),
                    )
                )
                account = env.db.impl().session.query(models.Product2BankAccount).get((user.bank_account_id, 1))
...
def get_consumers() -> typing.Tuple[models.User, dict[str, str]]:
    users = env.db.impl().session.execute(orm.select(models.User)).scalars().all()

    result = []
    for user in users:
        mapper, count = {}, 0
        for cat in norms.keys():
            status, payload = models.Consumption.did_consume_enough(user.bank_account_id, cat, norms[cat], time_accounted[cat])
...
def view_consumers():
    ...
    if not category.lower().startswith("all"):
        data = [(user, {category: mapper.get(category, "")}, lower) for user, mapper, lower in data]
        current = [category]
...
def view_consumption():
    ...
    consumption_data = (
        ...
        .filter(models.Consumption.bank_account_id == account)
        .order_by(orm.desc(models.Consumption.consumed_at))
        .all()
    )
    ...
    for consumed, product in consumption_data:
        row = {}
        if datetime.time.max > consumed.consumed_at.time() > datetime.time(hour=18):
            row["Автоматическое списание"] = "да"
            row["Цена автоматического списания"] = norms[product.category] * defaults[product.category]
            row["Время потребления"] = (consumed.local_consumed_at + datetime.timedelta(days=1)).strftime("%d/%m/%Y %H:%M:%S")
        else:
            row["Автоматическое списание"] = "нет"
            row["Цена автоматического списания"] = "-"
            row["Время потребления"] = consumed.local_consumed_at.strftime("%d/%m/%Y %H:%M:%S")
        row["Категория"] = product.category
        payload.append(row)

    return flask.render_template("main/view_all_consumption.html", payload=payload)

...
export default function CityHallPage() {
	...
	useEffect(() => {
		...
		(async () => {
			...
			const cityHallResp = await Hope.send(Request.create({ cityHall: cityHallReq }));
			...
			if (g) {
				...
				if (userIsMayor) {
					navigate(
						`/city_hall/${cityHall.bankAccountId}/goal/new?current=${money?.count ?? 0}`
					);
					return;
				}
				setGoal(null);
			...
			{isEcoAssistant && (
				<Stack spacing={2} sx={{ mt: 4 }}>
					<Button
						...
						size="large"
						onClick={() =>
							handleNavigate(`/city_hall/${cityHallBankAccount}/proposal/money`)
						}
					>
						Перевод средств
					</Button>
					...
					<Button
						...
						size="large"
						onClick={() =>
							handleNavigate(`/city_hall/${cityHallBankAccount}/proposal/history`)
						}
					>
						История транзакций
					</Button>

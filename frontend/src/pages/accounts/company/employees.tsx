...
import {
	...
	FireResponse,
	FireResponse_Status,
} from '@app/codegen/app/protos/company/fire';
										...
										<TableCell>
											{canFire ? (
												<Button
													...
													color="error"
													onClick={() =>
														onFire(e.info?.bankAccountId ?? 0, e.role)
													}
												>
													Уволить
												</Button>
...
export default function CompanyEmployeesPage() {
	...
	const fetchEmployees = async () => {
		...
		const response = (await Hope.send(Request.create({ allEmployees: req }))) as {
	...
	const handleFire = async (accountId: number) => {
		...
		const response = (await Hope.send(Request.create({ fire: fireReq }))) as {
			fire?: FireResponse;
		};

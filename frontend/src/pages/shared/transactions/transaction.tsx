...
const MoneyTransferForm: React.FC<MoneyTransferFormProps> = ({ mode }) => {
	...
	const userBankAccount = mode === PageMode.Org ? Number(params.orgId) : bankAccountId;
	...
	const getStatusMessage = (
		...
		switch (status) {
			...
			case TxStatus.AMOUNT_OUT_OF_BOUNDS:
				return {
					contents: 'Ошибка: сумма вне допустимого диапазона',
					status: AlertStatus.Error,
				};
			...
			case TxStatus.SELLER_MISSING_GOODS:
				return {
					contents: 'Ошибка: недостаточно средств на вашем счёте',
					status: AlertStatus.Error,
				};
			default:
				return {
					contents: 'Неизвестная ошибка при создании перевода',

...
const NewTransactionForm: React.FC<NewTransactionFormProps> = ({ mode }) => {
	...
	const getCreateTransactionStatus = (
		...
		switch (status) {
			...
			case TxStatus.CUSTOMER_IS_SELLER:
				return {
					contents: 'Ошибка: покупатель и продавец совпадают',
			...
			case TxStatus.COUNT_OUT_OF_BOUNDS:
				return {
					contents: 'Ошибка: некорректное количество товара',
					status: AlertStatus.Error,
				};
			...
			case TxStatus.SELLER_MISSING_GOODS:
				return {
					contents: 'Ошибка: у продавца недостаточно товара',
					status: AlertStatus.Error,
				};
			case TxStatus.CUSTOMER_MISSING_MONEY:
				return {
					contents: 'Ошибка: у покупателя недостаточно средств',
					status: AlertStatus.Error,
				};
	...
	return (
		<Box sx={{ maxWidth: 500, mx: 'auto', mt: 4, overflowX: 'auto' }}>
